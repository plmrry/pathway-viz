...
For example, the Reactome~\cite{croft2014reactome} displays a list of publications which are related to the selected item, see figure \ref{fig:reactome}, but as a simple list in a separate window adjacent to the visualization.
...
In this case, the structure of the graph does not change, only entity attributes (which Ahn \textit{et al.}~\cite{Ahn2014} refer to as the domain properties).
...
One specific case where implicit encoding is not mixed with other approaches is when when a graph is dynamic and the changes are between time slices.
...
Lee et al.~\cite{Lee2006} do describe a set of attribute based tasks in their taxonomy, but there is no aspect of causality or any sort of attribute cascade, only tasks related to static attribute values.
