...
This is functionality is available by default in \textit{Cytoscape} (and hence all of the associated plug-ins) via simply arranging the windows which display the networks.
\textit{Cerebral} uses a juxtaposition approach to display changes in attributes associated with the graph.
...
The \textit{RenoDoI} plugin uses superposition as a comparison technique, allowing multiple networks to be visualized in a single image. Bounding isocontours are used to distinguish graphs differences, and to clearly indicate where the graphs overlap.
...
This type of encoding of set membership is the \textit{Bubble Sets}~\cite{Collins2009} approach, which was shown to be the most effective way of displaying group information on a node-link diagram by Jianu et al.~\cite{Jianu2014}. Paduano et al. introduce the \textit{BranchingSets} technique~\cite{Paduano2016_VINCI,biovisPaduano1}, which makes it possible to see hierarchical information directly within a node of a biological pathway network, indicating nested proteins and protein complexes via user interaction. At the same time, a more elaborate tree view appears, highlighting the more complex hierarchical structure within the pathways. 
...
This approach has been used by the \textit{Cerebral} application for showing cascades of data.
...
While many visualization taxonomies exist, they tend to (intentionally) over-generalize, with the aim of describing tasks related to network data visualization in \textit{any} context.
