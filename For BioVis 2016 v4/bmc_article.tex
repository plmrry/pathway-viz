...
]{\inits{PM}\fnm{Paul} \snm{Murray}}
\author[
addressref={aff2},
email={fintan.mcgee@list.lu}
]{\inits{FM}\fnm{Fintan} \snm{McGee}}
\author[
addressref={aff1},
email={aforbes@uic.edu}
]{\inits{AGF}\fnm{Angus G.} \snm{Forbes}}

\address[id=aff1]{
  \orgname{Electronic Visualization Laboratory, University of Illinois at Chicago, Chicago, IL, USA}
  %M/C 152, University of Illinois at Chicago, 851 S. Morgan, Room 1120, Chicago 60607-7053, IL, USA
}

\address[id=aff2]{
  \orgname{eScience Group, Environmental Research and Innovation (ERIN) department, 
Luxembourg Institute of Science and Technology, Esch/Alzette, Luxembourg}
}

%\address[id=aff3]{\orgname{\url{http://creativecoding.evl.uic.edu}}}
...
A comprehensive understanding of tasks performed by domain researchers in a typical analysis is essential to the design and implementation of an effective visual analytics application~\cite{brehmer2013multi}.
...
Visualization techniques and applications are essential tools for researchers who work with complex data, and biological pathways are an active area of visualization research.
...
However, this paper is not intended to be a complete survey of biological visualization techniques and applications. %Existing techniques included in this paper %These existing techniques include the following: 
Here we look at tools that exemplify typical visualization strategies, including:
\textit{ChiBe}~\cite{Babur2010chibe}, \textit{Entourage}~\cite{Lex2013entourage}, \textit{Reactome Pathway Browser}~\cite{croft2014reactome}, \textit{VisAnt}~\cite{hu2004visant}, \textit{MetaViz}~\cite{bourqui2007metabolic}, and \textit{VitaPad}~\cite{holford2005vitapad}.

Node-link diagrams are the nearly-universal choice of visual representation used in existing applications (exceptions to this rule include \textit{BioFabric}~\cite{Longabaugh2012biofabric}).
...
However, node-link representations are one of several ways to visualize graph data, and there are alternative visualization techniques which can be applied to pathway data~\cite{biovisDang1,biovisDang2}.
...
While matrix techniques are not as effective for certain tasks (such as path-tracing), linked views and hybrid techniques exist, such as \textit{NodeTrix}~\cite{NodeTrix2007}, which combine node-link and matrix representations.
...
\textit{Systems Biology Graph Notation} (SBGN)~\cite{Novere2009} is a visual standard often used to visualize \textit{BioPAX} and \textit{SBML} file formats.
Features particular to \textit{SBGN} include the definition of multiple edge and node types, as well as allowing edges to connect to more than two nodes, resulting in a hypergraph.
...
Brehmer and Munzner~\cite{brehmer2013multi} extend these abstractions by linking high-level and low-level tasks into a multi-level typology, which greatly extends the usefulness of a visualization taxonomy, allowing it to be applied to a wide variety of visualization domains.
...
The complex nature of dynamic graph data yields a similarly complex set of analysis tasks, and many of these tasks were not covered by the general graph taxonomy of Lee et al. --- thus, new tasks needed to be specified.
...
Their work builds on the work of both Lee et al. and of Valiati et al.~\cite{Valiati2006}, as multivariate networks can be considered a multidimensional dataset.
...
While Saraiya et al.'s~\cite{saraiya2005visualizing} objectives are similar to ours,
...
%%% AGF edited this, this is original version
...
Interviews were conducted with seven domain experts in biology, each of whom works with pathway data in some form. A summary of the interviews is described in Table~\ref{table:interviews}. The domain experts are engaged in a wide variety of research within the general domain of biology and bioinformatics research, but all of which have some relationship to pathway data.
...
The interviews were free-form discussions aimed at understanding the research process of each domain expert, the tasks performed by the researcher in the course of a typical analysis, and, importantly, the structure and content of the data used in their published research.
They were intentionally open-ended, and were designed to capture a variety of tasks that are seen as important to domain experts.
...
%% Table captions on top in journal version
 ...
 \begin{center}
   \begin{tabular}{|p{6.5cm}|}
     ...
     \\ % Qun-Tian Wang
     \hline \textbf{Title:} Assistant Professor \T \\
     \textbf{Domain:} Computational and Systems Biology \\
     \textbf{Studies:} Cancer cell death \B
     \\ % Guang Yao
     \hline \textbf{Title:} Assistant Professor \T \\
     \textbf{Domain:} Bioinformatics and Systems Biology \\
     \textbf{Studies:} Evolution, genetic network topology, population genomics \B
     \\ % Ryan Gutenkunst
...
Our taxonomy divides tasks into three broad categories: Attribute, Relation, and Modification tasks.
The attribute category includes the identification of attributes (A1), comparison of attributes (A2), and the identification of provenance and uncertainty (A3 and A4).
The relationship category includes the identification of relationship attributes (R1), directed relationships (R2), and grouped relationships (R3), as well as the identification of causality, cascading effects, and feedback loops (R4 and R5).
The modification category includes tasks related to updating and curating data, including collaborative annotation (M1) and curation (M2).
...
\ \\ \
\noindent{\textbf{(A1)} \underline{Identify Multivariate Attributes}}
...
Indeed, the identification (and comparison) of attributes is closely coupled with the identification (and comparison) of overall topological structure~\cite{partl2012enroute}.

An additional concern with biological attribute data is the biological \emph{context} of an entity (e.g., a tissue, organ, or species), especially when datasets can contain similar entity types that were measured across a variety of different contexts.
...
The \textit{RenoDoI} application~\cite{Vehlow2015}, a plug-in for \textit{Cytoscape} for visualizing knowledge networks of biological data, uses ``degree of interest'' functions to highlight nodes based on attribute values.
...
Node and edge attributes are also used for graph querying and filtering as can be seen in facet-based visualizations, an approach that allows for graphs to be filtered by subsets of attributes.
...
\multicolumn{1}{l|}{} & \textbf{(A1)} Multivariate & Find all up-regulated genes in a biological pathway. Integrate results of a laboratory experiment into existing protein-protein interaction networks. \\ \cline{2-3} %, extracted from StringDB.
...
\multirow{5}{*} {} & \textbf{(R1)} Attributes &	Find all translocations of entities in a given biological pathway. \\ \cline{2-3}
...
& \textbf{(M1)} Annotate   & Update out-of date-information in a pathway data set, or create a personalized pathway relevant to a specialized research topic. \\ \cline{2-3}
...
\ \\ \
\noindent{\textbf{(A2)} \underline{Compare Attributes}}
...
This is functionality is available by default in \textit{Cytoscape}~\cite{Shannon2003cytoscape} (and hence all of the associated plug-ins) via simply arranging the windows which display the networks.
...
Within \textit{Cytoscape} there are several ways to map graphical attributes to data, to allow for data from different data sets to be visualized differently.
The \textit{RenoDoI} plugin~\cite{Vehlow2015} uses superposition as a comparison technique, allowing multiple networks to be visualized in a single image. Bounding isocontours are used to distinguish graphs differences, and to clearly indicate where the graphs overlap.
Graph layout is an important aspect of both juxtaposition and superposition based comparative visualizations.
...
The \textit{RenoDoI} application initially lays out the largest data set, then adds the additional data sets, adjusting the previous layout without resetting it.
Nodes which are included in both data sets only appear once.

Explicit encoding of difference means that differences between the two datasets are explicitly highlighted, and this approach is often provided in addition to the previous two. For example, an edge which appears in one data set but not the other may be highlighted by color.
...
This can be seen in Rugfiange and McGuffins's \textit{DiffAni} application~\cite{Rufiange2013} for visualizing dynamic graphs.


\ \\ \
\noindent{\textbf{(A3, A4)} \underline{Identify Provenance and Uncertainty}}
...
The provenance can refer to the type of source, such as a peer reviewed publication, experimental results, or a textual analysis.
...
For example, each relationship within a \textit{BioPAX} file is usually associated with a publication that provides evidence for its existence.
...
In their state-of-the-art report on the visualization of group structures in graphs, Vehlow et al.~\cite{vehlow2015state} discuss uncertainty as one of several ongoing research challenges.
...
While \textit{SBGNViz}~\cite{SBGNViz2015} and \textit{ChiBE}~\cite{Babur2010chibe} and other applications allow connectivity to external sources, such as \textit{UniProt} or \textit{PubMed}, there are few biological visualization tools that visualize provenance information directly.

Most online biology databases do provide this information but do not integrate it into the data visualization itself.
For example, \textit{Reactome}~\cite{croft2014reactome} displays a list of publications which are related to the selected item as a simple list in a separate window adjacent to the visualization.

\textit{STRING}~\cite{STRING2005}, a protein interaction database, provides provenance information and incorporates it into its associated visualization.
The provenance is described with respect to its source (e.g., experimental results or a curated database) and is encoded by color within the database's visualization component. \textit{BranchingSets}~\cite{Paduano2016_VINCI} uses multi-colored links and nodes to indicate the provenance of specific proteins and biochemical relationships between proteins, making it easier for a user to see which contexts are relevant for particular elements. \textit{TimeArcs}~\cite{Dang2016a_IEEEEuroVis} is a visualization technique that highlights PubMed articles related to particular subnetworks of proteins within a specified time range. At a glance, a user can see whether or not a particular protein or set of proteins is described within the literature of biological pathways. Moreover, he or she can see if the relationships between each of these proteins is confirmed or contradicted by successive publications, indicating, for example, further details about known pathways, or that in different contexts (e.g., tissues, organs, or species) pathways exhibit different functionality. 

Some databases also provide quality scores with their results.
...
While their approach visualizes potential ambiguity in visual interpretation rather than within the underlying data set, a similar approach could be taken to visualize uncertainty in biological networks.
...
\ \\ \
\noindent{\textbf{(R1)} \underline{Identify Relationship Attributes}}
...
Pretorius and van Wijk's~\cite{pretorius2008} system for visual inspection of multivariate graphs places the relationship type (referred to as edge labels) at the core of their system.
...
Ghani et al.~\cite{Ghani2013} developed a techniques called \textit{Parallel Node-Link Bands} (PNLBs) for exploring graphs with multiple edge types.
...
\ \\ \
\noindent{\textbf{(R2)} \underline{Identify Directed Relationships}}
...
The graphs used in Holten and van Wijk's are simple directed graphs.
Biological pathways are usually modeled as hyper-graphs, with many different types of edges and hyperedges.
Visual encodings such as \textit{SBGN} and \textit{KEGG} contain many different visual representations for edges, so applying the tapered edge visualization style to complex biological pathways is not trivial and would require an empirical evaluation.
However, the results of Holten and van Wijk's work suggest that investigating such an approach may be worthwhile.


\ \\ \
\noindent{\textbf{(R3)} \underline{Identify Grouping / Hierarchical Relationships}}
...
A biochemical reaction could be connected, via a single \emph{NextStep} relationship, to an entire pathway, which could potentially contain thousands of nodes.
This relationship is clearly not the same as a biochemical reaction being connected to every entity within a pathway.
This example also demonstrates the distinction between a compound relationship and a hierarchical relationship (which are two types of grouping relationships).
...
The \textit{BranchingSets} technique~\cite{Paduano2016_VINCI,biovisPaduano1} facilitates the exploration of hierarchical information in biological pathways, which is presented directly within the nodes in  network. At a glance, a user can see an overview of the nested structure of a protein complex, and user interaction brings up a more elaborate tree view that provides further details about a selected complex, highlighting the hierarchical patterns within a set of pathways.

\ \\ \
\noindent{\textbf{(R4, R5)} \underline{Identify Causality and Cascading Effects}}
...
In tandem with the problem of identifying cascading effects is the problem of reasoning about feedback~\cite{biovisDang2}.
...
This approach has been used by the \textit{Cerebral} application for showing cascades of data~\cite{Barsky2008cerebral}.
...
Additionally, the authors of \emph{enRoute}~\cite{partl2012enroute} briefly discuss a case study in which their tool can be used for the visualization of causality in the context of experimental results.
...
\ \\ \
\noindent{\textbf{(M1, M2)} \underline{Annotate and Curate}}
\paragraph*{Description}
...
\paragraph*{Existing Approaches and Techniques}
Most desktop pathway visualization applications allow for data files to be edited and exported in standardized formats, e.g., \textit{CellDesigner}~\cite{funahashi2008celldesigner} allows files to be modified and curated and exported in the \textit{SBML} standard.
...
\textit{Reactome}~\cite{croft2014reactome} and \textit{STRING}~\cite{STRING2005} are two online resources which feature highly interactive web based visualization interfaces.
The fact that the data in the \textit{Reactome} database is curated and peer reviewed is considered an important feature of the system.
...
In their overview of dynamic network visualization, Moody et al.~\cite{moody2005dynamic} do mention cascades, but only in the context of the formation of relationships, not the changing of attributes.
...
In addition, a second round of explicitly structured interviews will be a valuable next step in the continued refinement of this taxonomy. %We would also greatly benefit from continued feedback from domain experts on the tasks that we have highlighted here, and to determine if there are any gaps in our current taxonomy.
...
%tend to (intentionally) over-generalize, with the aim of 
While relevant network visualization taxonomies exist, they largely describe tasks related to network data visualization in \textit{any} context.
...
Existing visualization contexts are often deliberately generalized, and are meant to address challenges and provide guidelines for the visualization of certain categories of \textit{data}, rather than certain categories of \textit{research}.
...
Through interviews with domain experts, we have identified tasks important to researchers who work with biological pathway data, and this taxonomy acts as a reference point for the large and growing field of biological pathway visualization.
...
%We thank the domain experts who shared their expertise by participating in interviews and discussions during the development of this work.
...
\section*{Funding}
This publication is based on work supported by the DARPA \emph{Big Mechanism} Program under ARO contract WF911NF-14-1-0395, by the ANR grant BLIZAAR ANR-15-CE23-0002-01, and by the FNR grant BLIZAAR INTER/ANR/14/9909176.

\section*{Authors' contributions}
PM conducted the interviews with the biologists and bioinformaticians and created the initial task taxonomy for biological pathway network visualization tasks. FM and AGF helped to refine the task definitions and provide additional examples. PM and FM analyzed relevant visualization software tools and techniques. All authors read and approved the final manuscript.
