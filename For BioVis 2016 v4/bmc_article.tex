...
While these matrix techniques are not as effective for certain tasks (such as path-tracing), linked views and hybrid techniques exist, such as \textit{NodeTrix}~\cite{NodeTrix2007}, which combine node-link and matrix representations.
...
Interviews were conducted with seven domain experts in biology, each of whom works with pathway data in some form. A summary of the interviews is described in Table~\ref{table:interviews}.
...
The relationship category includes the identification of relationship attributes (R1), directed relationships (R2), and grouped relationships (R3), as well as the identification of causality, cascading effects, and feedback loops.
...
The \textit{RenoDoI} application~\cite{Vehlow2015} initially lays out the largest data set, then adds the additional data sets, adjusting previous layout , without resetting it.
...
In their state-of-the-art report on the visualization of group structures in graphs, Vehlow et al.~\cite{vehlow2015state} discuss uncertainty as one of several ``Research Challenges.''
...
This publication is based on work supported by the DARPA \emph{Big Mechanism} Program under ARO contract WF911NF-14-1-0395, and work supported by the ANR grant BLIZAAR ANR-15-CE23-0002-01 and the FNR grant BLIZAAR INTER/ANR/14/9909176.
